...
export const sampleQuestions: Record<SegmentCode, Question[]> = {
  ...
  AUCT: [
    ...
    },
    {
      ...
      answers: ['مانشستر يونايتد', 'يونايتد', 'Manchester United', 'Man United'],
      difficulty: 'medium',
      points: 2
    },
    {
...
// Helper functions
export const getQuestionsForSegment = (segment: SegmentCode, count: number = 5): Question[] => {
  const questions = sampleQuestions[segment] || [];
  return questions.slice(0, count);
};

export const getRandomQuestions = (segment: SegmentCode, count: number = 5): Question[] => {
  const questions = sampleQuestions[segment] || [];
